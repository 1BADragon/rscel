...
mod tests {
    ...
    fn cel_to_sql(cel_expr: &str) -> Result<String, ToSqlError> {
        ...
        let builder = program.ast().unwrap().into_sql_builder()?;
        builder.to_sql()
    }
    ...
    #[test_case("true ? 'yes' : 'no'", "case (TRUE)::bool when true then ('yes') else ('no') end"; "simple ternary")]
    #[test_case("x > 5 ? 10 : 0", "case ((x) > (5))::bool when true then (10) else (0) end"; "ternary with comparison")]
    #[test_case("(x > 0 ? true : false) ? 'positive' : 'non-positive'", "case ((case ((x) > (0))::bool when true then (TRUE) else (FALSE) end))::bool when true then ('positive') else ('non-positive') end"; "nested ternary condition")]
    #[test_case("age >= 18 ? 'adult' : 'minor'", "case ((age) >= (18))::bool when true then ('adult') else ('minor') end"; "age check ternary")]
    ...
    #[test_case("x > 0 ? x * 2 : x / 2", "case ((x) > (0))::bool when true then ((x) * (2)) else ((x) / (2)) end"; "ternary with arithmetic")]
    #[test_case("((a + b) * (c - d)) == ((e / f) + (g % h))", "(((((a) + (b))) * (((c) - (d))))) = (((((e) / (f))) + (((g) % (h)))))"; "multiple levels of nesting")]
    #[test_case("(x > 5 ? true : false) && (y < 10 || z == 0)", "((case ((x) > (5))::bool when true then (TRUE) else (FALSE) end)) AND ((((y) < (10)) OR ((z) = (0))))"; "complex logical with ternary")]
    ...
    #[test_case("status == 'active' ? priority : 0", "case ((status) = ('active'))::bool when true then (priority) else (0) end"; "status check with fallback")]
    ...
    #[test_case("user_type == 'admin' ? full_access : (department == 'IT' ? tech_access : basic_access)", "case ((user_type) = ('admin'))::bool when true then (full_access) else ((case ((department) = ('IT'))::bool when true then (tech_access) else (basic_access) end)) end"; "nested conditions")]
    ...
    #[test_case("x > 0 ? int(x) : int(0)", "case ((x) > (0))::bool when true then (x::integer) else (0::integer) end"; "casting in ternary")]
    ...
    #[test_case("[x > 0 ? 1 : 0, y > 0 ? 'pos' : 'neg']", "ARRAY[case ((x) > (0))::bool when true then (1) else (0) end, case ((y) > (0))::bool when true then ('pos') else ('neg') end]"; "array with ternary")]
    ...
    #[test_case("x > 0 ? [1, 2] : [3, 4]", "case ((x) > (0))::bool when true then (ARRAY[1, 2]) else (ARRAY[3, 4]) end"; "array in ternary")]
    ...
    #[test_case("{'name': 'John', 'age': 30}", "json_build_object('name', 'John', 'age', 30)"; "object with mixed types")]
    #[test_case("{'active': true, 'count': 42}", "json_build_object('active', TRUE, 'count', 42)"; "object with boolean and integer")]
    #[test_case("{'data': null, 'valid': false}", "json_build_object('data', NULL, 'valid', FALSE)"; "object with null and boolean")]
    #[test_case("{'nested': {'inner': 'value'}}", "json_build_object('nested', json_build_object('inner', 'value'))"; "nested object")]
    #[test_case("{'x': 1, 'y': 2, 'z': 3}", "json_build_object('x', 1, 'y', 2, 'z', 3)"; "object with multiple fields")]
    ...
    #[test_case("{'sum': x + y, 'diff': a - b}", "json_build_object('sum', (x) + (y), 'diff', (a) - (b))"; "object with arithmetic expressions")]
    #[test_case("{'result': x > 5, 'status': 'ok'}", "json_build_object('result', (x) > (5), 'status', 'ok')"; "object with comparison")]
    #[test_case("{'value': int('42'), 'text': string(123)}", "json_build_object('value', '42'::integer, 'text', 123::text)"; "object with type casting")]
    #[test_case("{'condition': x ? 'yes' : 'no'}", "json_build_object('condition', case (x)::bool when true then ('yes') else ('no') end)"; "object with ternary")]
    #[test_case("{'func': max(a, b), 'array': [1, 2, 3]}", "json_build_object('func', max(a, b), 'array', ARRAY[1, 2, 3])"; "object with function and array")]
    ...
    #[test_case("data.enabled ? 'yes' : 'no'", "case ((data)->>'enabled')::bool when true then ('yes') else ('no') end"; "member access in ternary condition")]
