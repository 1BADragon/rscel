...
#[test_case("\"abc123\".matches(\"[a-z]{3}[0-9]{3}\")", true.into(); "test matches method")]
#[test_case("matches('abc123', '[a-z]{3}[0-9]{3}')", true.into(); "test matches function")]
...
#[test_case("log(1)", 0.into(); "log")]
#[test_case("log(1u)", 0u64.into(); "log unsigned")]
...
#[test_case("timestamp('2023-12-25T12:00:00Z').getDayOfMonth()", 24.into(); "getDayOfMonth")]
#[test_case("timestamp('2023-12-25T7:00:00Z').getDayOfMonth('America/Los_Angeles')", 23.into(); "getDayOfMonth with timezone")]
#[test_case("int(1)", 1.into(); "identity -- int")]
#[test_case("uint(1u)", 1u64.into(); "identity -- uint")]
#[test_case("double(5.5)", 5.5.into(); "identity -- double")]
#[test_case("string('hello')", "hello".into(); "identity -- string")]
#[test_case("bytes(bytes('abc'))", crate::types::CelBytes::from_vec(vec![97u8, 98u8, 99u8]).into(); "identity -- bytes 1")]
#[test_case("bytes(b'abc')", crate::types::CelBytes::from_vec(vec![97u8, 98u8, 99u8]).into(); "identity -- bytes 2")]
#[test_case("duration(duration('100s')) == duration('100s')", true.into(); "identity -- duration")]
#[test_case("duration('2h') + duration('1h1m') >= duration('3h')", true.into(); "duration add + comp")]
#[test_case("timestamp(timestamp(100000000)) == timestamp(100000000)", true.into(); "identity -- timestamp")]
#[test_case("bool(true)", true.into(); "bool true")]
#[test_case("bool(false)", false.into(); "bool false")]
#[test_case("bool('1')", true.into(); "'1' -> bool")]
#[test_case("bool('t')", true.into(); "'t' -> bool")]
#[test_case("bool('true')", true.into(); "'true' -> bool 1")]
#[test_case("bool('TRUE')", true.into(); "'TRUE' -> bool 2")]
#[test_case("bool('True')", true.into(); "'True' -> bool 3")]
#[test_case("bool('0')", false.into(); "'0' -> bool")]
#[test_case("bool('f')", false.into(); "'f' -> bool")]
#[test_case("bool('false')", false.into(); "'false' -> bool 1")]
#[test_case("bool('FALSE')", false.into(); "'FALSE' -> bool 2")]
#[test_case("bool('False')", false.into(); "'False' -> bool 3")]
#[test_case("!true", false.into(); "not true")]
#[test_case("!false", true.into(); "not false")]
#[test_case("1 + 2 == 3 && 4 + 5 == 9", true.into(); "and operation with expressions")]
#[test_case("1 + 2 == 3 || 4 + 5 == 10", true.into(); "or operation with expressions")]
#[test_case("! (1 + 2 == 4)", true.into(); "negated expression")]
#[test_case("size([1, 2, 3].filter(x, x > 1)) == 2", true.into(); "filter and size")]
#[test_case("max(1, 2, 3) + min(4, 5, 6) == 7", true.into(); "max and min")]
#[test_case("['hello', 'world'].reduce(curr, next, curr + ' ' + next, '')", " hello world".into(); "reduce with strings")]
#[test_case("timestamp('2024-07-30 12:00:00Z') > timestamp('2023-07-30 12:00:00Z')", true.into(); "timestamp comparison")]
#[test_case("{'a': 1, 'b': 2, 'c': 3}.filter(k, k == 'b').size() == 1", true.into(); "filter on map with modulo")]
